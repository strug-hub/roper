...
library("edgeR")
library(roper)
source("./runScripts_rope.R")

library(methods)

algos <- list(
  "DESeq2" = runDESeq2Outliers,
  "edgeR" = runEdgeR,
...
methods <- c("DESeq2", "edgeR", "edgeR-robust", "limma-voom", "RoPE")
...
ms <- c(60, 100, 200, 300, 400)

nreps <- 50
# nreps <- 3

res_nb <- do.call(rbind, lapply(ms, function(m) {
  do.call(rbind, lapply(percentOutliers, function(pOut) {
    ...
    resList <- lapply(seq_len(nreps), function(i) {
      ...
      e <-
        ExpressionSet(mat, AnnotatedDataFrame(data.frame(condition)))
      resTest0 <- lapply(algos, function(f) {
        f(e)
      })
      resTest <- list()
      resTest[names(resTest0[["DESeq2"]])] <- resTest0[["DESeq2"]]
      resTest[c("edgeR", "edgeR-robust", "limma-voom", "RoPE")] <-
        resTest0[c("edgeR", "edgeR-robust", "limma-voom", "RoPE")]
      resTest[["beta"]] <- beta
      ...
      padjMat <- sapply(seq_along(resList), function(i) {
        sapply(pvalsVector, function(p) {
          ...
          if (sum(smallp) == 0) {
            ...
            max(resList[[i]][[method]]$padj[idx][smallp])
          }
        })
      })
...
# save(res_nb, file="./results_simulateOutliers_nb_v6.RData")
